...
def cumsum_buy_sell_volume_interval_directionalised(
    ...
    rolling_wd_volume = volume_df.volume.rolling(
        window=pd.Timedelta(seconds=interval)
    ).sum()
...
def flow_of_larger_orders_in_interval(
    ...
    rolling_wd_largevolume = largevolume_df.largevolume.rolling(
        window=pd.Timedelta(seconds=interval)
    ).sum()
...
def flow_of_smaller_orders_in_interval(
    ...
    rolling_wd_smallvolume = smallvolume_df.largevolume.rolling(
        window=pd.Timedelta(seconds=interval)
    ).sum()
...
def level_thickness_rank(
    ...
    if side == "support":
        ...
        total_volume_units = (
            bbov_weights[0] * support_units_1
            + bbov_weights[1] * support_units_0
            + bbov_weights[2] * resistance_units_0
            + bbov_weights[3] * resistance_units_1
        )
    ...
    volume_df.drop(columns="timeindex", inplace=True)
    rolling_rank = (
        volume_df.volume.rolling(window=pd.Timedelta(seconds=interval)).rank()
...
def buy_sell_pct_interval_directionalised(
    ...
    rolling_df = pd.DataFrame(
        {"cdelta": cdelta, "cdelta_total": cdelta_total, "timeindex": timestamps}
    )
    rolling_df.index = pd.to_datetime(rolling_df.timeindex, unit="ns")
    rolling_df.drop(columns="timeindex", inplace=True)
    rolling_volume_pct = (
        rolling_df.cdelta.rolling(window=pd.Timedelta(seconds=interval)).sum()
        / rolling_df.cdelta_total.rolling(window=pd.Timedelta(seconds=interval)).sum()
    )

    directionalised_rolling_volume_pct = (
        rolling_volume_pct * trigger_direction
        if side == "buy"
        else rolling_volume_pct * trigger_direction * -1
    )
...
def turnover_impulse(
    ...
    if mode == "mean":
        rolling_cdelta = cdelta_df.cdelta.rolling(
            window=pd.Timedelta(seconds=interval1)
        ).mean()
        rolling_cdelta2 = cdelta_df.cdelta.rolling(
            window=pd.Timedelta(seconds=interval2)
        ).mean()
    elif mode == "median":
        rolling_cdelta = cdelta_df.cdelta.rolling(
            window=pd.Timedelta(seconds=interval1)
