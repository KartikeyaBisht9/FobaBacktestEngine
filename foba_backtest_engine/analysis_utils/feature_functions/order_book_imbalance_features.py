...
def order_book_imbalance1(
    ...
    return (bid_0_volume / ask_0_volume) - 1.0
...
def order_book_imbalance2(
    ...
    return ((bid_0_volume + bid_1_volume) / (ask_0_volume + ask_1_volume)) - 1.0
...
def order_book_imbalance_interval_mean(
    ...
    if type == "one":
        ...
        OBI = order_book_imbalance2(
            bid_0_volume, bid_1_volume, ask_0_volume, ask_1_volume
        )
    else:
        raise NotImplementedError
    OBI_df = pd.DataFrame({"OBI": OBI, "timeindex": timestamps})
    OBI_df = (
        OBI_df.groupby("timeindex")
        .agg({"OBI": "mean", "timeindex": "mean"})
    ...
    rolling_df = pd.DataFrame(
        {"timestamp": new_timestamps, "rolling_result": rolling_result["OBI"].values}
    )
...
def order_book_imbalance_interval_argmax_diff(
    ...
    argmax_diff = OBI_df.rolling(window=pd.Timedelta(seconds=interval)).apply(
        lambda arr: (arr.shape[0] - np.argmax(arr)) / float(arr.shape[0]),
        raw=True,
        engine="numba",
    ...
    rolling_df = pd.DataFrame(
        {"timestamp": new_timestamps, "rolling_result": argmax_diff["OBI"].values}
    )
...
def order_book_imbalance_interval_std_direction(
    ...
    if threshold == "by_sign":
        if direction == "positive":
            ...
            OBI_df["OBI"] = OBI_df["OBI"].apply(lambda x: x if x < thr else np.nan)
        rolling_std = (
            OBI_df.rolling(window=pd.Timedelta(seconds=interval)).std().fillna(0)
        )

        rolling_df = pd.DataFrame(
            {"timestamp": new_timestamps, "rolling_result": rolling_std["OBI"].values}
        )
        ...
        rolling_std = (
            OBI_df.rolling(window=pd.Timedelta(seconds=interval))
            .apply(
                lambda arr: np.nanstd(arr[arr > np.nanmean(arr)])
                if direction == "positive"
                else np.nanstd(arr[arr < np.nanmean(arr)]),
                raw=True,
                engine="numba",
            )
            .fillna(0)
        ...
        rolling_std = (
            OBI_df.rolling(window=pd.Timedelta(seconds=interval))
            .apply(
                lambda arr: np.nanstd(arr[arr > np.nanmedian(arr)])
                if direction == "positive"
                else np.nanstd(arr[arr < np.nanmedian(arr)]),
                raw=True,
                engine="numba",
            )
            .fillna(0)
