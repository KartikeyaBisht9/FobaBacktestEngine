...
import pandas as pd
from foba_backtest_engine.analysis_utils.feature_functions.decayed_returns import decayed_returns
...
def vwap_over_ref_price(
    ...
    volume_df = pd.DataFrame({'volume': volume, 'cdelta': cdelta, 'timeindex': timestamps})
    volume_df.index = pd.to_datetime(volume_df.timeindex, unit='ns')
    volume_df.drop(columns='timeindex', inplace=True)

    vwap = volume_df.cdelta.rolling(window=pd.Timedelta(seconds=interval)).sum() / volume_df.volume.rolling(window=pd.Timedelta(seconds=interval)).sum()
    vwap - price_value
...
def trend_strength(
    ...
    trend = price_df.price.rolling(window=pd.Timedelta(seconds=interval)).apply(
        lambda arr: 0 if (np.nansum(np.abs(np.diff(arr)))) == 0 else (arr[-1] - arr[0]) / (
            np.nansum(np.abs(np.diff(arr)))),
...
def price_volume_corr_directionalised(
    ...
    dr = decayed_returns(timestamps, trigger_direction, price_prev_row, price_value, dr_halflife)
    combined_df = pd.DataFrame({'ld_volume_directionalised': trigger_lds_with_direction, 'dr': dr, 'timeindex': timestamps})
    ld_df = combined_df.query('ld_volume_directionalised != 0')

    ld_df['implied_correlation'] = ld_df['ld_volume_directionalised'].rolling(window=pd.Timedelta(seconds=interval)).corr(ld_df['dr'])
    merged_df = pd.merge(combined_df, ld_df[['timeindex', 'implied_correlation']], on='timeindex', how='left')
    merged_df['implied_correlation'] = merged_df['implied_correlation'].fillna(method='ffill')
    del ld_df
    del combined_df
