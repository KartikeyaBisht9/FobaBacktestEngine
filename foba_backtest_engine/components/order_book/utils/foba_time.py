...
class SentTime(metaclass=ABCMeta):
    def __init__(
        self, time_profile, profile_name=None, times=("level", "join", "event")
    ):
...
class MinSentTime(SentTime):
    def calculate(self, exchange_timestamp, driver_received):
        return min_order_sent_time(
            exchange_timestamp, driver_received, self.time_profile
        )
...
TimeProfile = namedtuple(
    "TimeProfile",
    (
        "min_one_way_delay",
        "min_round_trip_delay",
        "max_one_way_delay",
...
omdc_profile = TimeProfile(
    min_one_way_delay=-float("inf"),
    min_round_trip_delay=0.35e6,
    max_one_way_delay=float("inf"),
    max_round_trip_delay=0.8e6,
)


def max_order_sent_time(exchange_published, driver_received, time_profile):
    return min(
        exchange_published - time_profile.min_one_way_delay,
        driver_received - time_profile.min_round_trip_delay,
    )


def min_order_sent_time(exchange_published, driver_received, time_profile):
    return max(
        exchange_published - time_profile.max_one_way_delay,
        driver_received - time_profile.max_round_trip_delay,
    )
...
class SentTimeCalculator:
    ...
    def calculate(self, event):
        return self._calculate(
            self._exchange_timestamp(event), self._driver_received(event)
        )
