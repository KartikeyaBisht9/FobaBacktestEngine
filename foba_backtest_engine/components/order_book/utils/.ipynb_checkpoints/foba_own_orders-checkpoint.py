...
def fetch_one_tick(date_to_inspect, symbols, tick="SST_TRADE", database='XHKG'):
    ...
    df = pd.read_feather("/Users/kartikeyabisht/FobaBacktestEngine/temp_data/trade_confirms.feather")
...
def fetch_order_info(date_to_inspect, feedcodes, data_store = "core_trades", operation='order_insert', mic='XHKG'):
    ...
    if operation == "order_insert":
        df = pd.read_feather("/Users/kartikeyabisht/FobaBacktestEngine/temp_data/addOrders.feather")
        df = df[df["feedcode"].isin(feedcodes)]
        pass
    elif operation == "private_trade":
        df = pd.read_feather("/Users/kartikeyabisht/FobaBacktestEngine/temp_data/private_trades.feather")
        df = df[df["feedcode"].isin(feedcodes)]
        pass
    elif operation == "delete_operation":
        df = pd.read_feather("/Users/kartikeyabisht/FobaBacktestEngine/temp_data/deleteOrders.feather")
        df = df[df["feedcode"].isin(feedcodes)]
        pass
    else:
...
def order_numbers_filtered(filter, type = 'list'):
    ...
    addOrders["datetime"] = pd.to_datetime(addOrders["exchange_timestamp"]+8*60*60*1e9)
...
def get_competitor_FAKS(filter, FAK_ids):
    ...
    how='inner')
    # TODO - fix
    combined["datetime"] = pd.to_datetime(combined["exchange_timestamp"]+8*60*60*1e9)
...
@enriches('foba_events')
def order_delete_matches(foba_events, optiver_order_deletes, full_feed_state_enrichment):
    def matches():
        ...
        for eventId, event in foba_events.items():
            ...
            if order_key in order_deletes:
                ...
                output = OptiverDeleteMatch(
                            ...
                            optiver_driver_sent=order.driver_sent_time,
                            optiver_exchange_time =order.exchange_timestamp,
                            volume_ahead_at_optiver_delete_sent=volume_ahead(event, order, feed_at_delete),
                            optiver_send_delay= order.one_way_latency
                        )
            else:
                output = OptiverDeleteMatch(
...
def optiver_trade_and_quotes(filter, foba_events, optiver_trades, order_delete_matches):
    quotes = {(trade.bookId_, trade.orderId_) : trade for key, trade in optiver_trades.items() if trade.optiver_hit == False}
    hits = {(trade.bookId_, trade.orderId_) : trade for key, trade in optiver_trades.items() if trade.optiver_hit == False}
        ...
        for event_id, event in foba_events.items():
            if (event.event_type == EventType.TRADE):
                ...
                if quote_key in quotes:
                    ...
                    hit, pickoff, add_delete_delay, competitor_speed = False, False, np.nan, np.nan
