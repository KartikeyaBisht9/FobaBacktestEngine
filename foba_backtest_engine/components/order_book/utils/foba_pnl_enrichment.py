...
def enrich_pnl(
    ...
    def items():
        for event_id, event in foba_events.items():
            ...
            slipped_pnl_fields = [f"aggressor_pnl_{x}s" for x in pnl_slippage_times] + [
                f"passive_pnl_{x}s" for x in pnl_slippage_times
            ]
            slipped_pnl_bps_fields = [
                f"aggressor_bps_{x}s" for x in pnl_slippage_times
            ] + [f"passive_bps_{x}s" for x in pnl_slippage_times]
            slipped_pnl_ticks_fields = [
                f"aggressor_tick_{x}s" for x in pnl_slippage_times
            ] + [f"passive_tick_{x}s" for x in pnl_slippage_times]
            all_fields = (
                slipped_pnl_fields + slipped_pnl_bps_fields + slipped_pnl_ticks_fields
            )
            ...
            for time in pnl_slippage_times:
                passive_pnl_ = np.where(
                    event.event_type == EventType.TRADE,
                    np.where(
                        event.side == Side.BID,
                        (getattr(feed_state, f"midspot_{time}") - event.event_price)
                        * (event.event_volume * static_data.contract_size),
                        (event.event_price - getattr(feed_state, f"midspot_{time}"))
                        * (event.event_volume * static_data.contract_size),
                    )
                    - static_data.fees,
                    0,
                ...
                aggro_pnl_bps = 10_000 * (
                    aggro_pnl / (event.event_volume * event.event_price)
                )
                passive_pnl_bps = 10_000 * (
                    passive_pnl_ / (event.event_volume * event.event_price)
                )

                aggro_pnl_ticks = 10_000 * (
                    aggro_pnl / event.event_volume / static_data.tick_size
                )
                passive_pnl_ticks = 10_000 * (
                    passive_pnl_ / event.event_volume / static_data.tick_size
                )
