...
def get_hk_broker_data_from_backup(filter):
    # path = "OMDC/Backup/BrokerMapping.parquet"
    # df = pd.read_parquet(
    #     "/Users/kartikeyabisht/FobaBacktestEngine/temp_data/BrokerMapping.parquet"
    # )
    df = pd.read_parquet(filter.broker_mapping_path)
...
def get_hk_broker_data_from_hkex():
    ...
    datareader = csv.reader(
        io.TextIOWrapper(webpage, encoding="utf-16"), delimiter="\t"
    )
...
def get_hk_broker_data(filter, brokermap_backup=True):
    if brokermap_backup:
        return get_hk_broker_data_from_backup(filter)
    else:
        try:
...
def omdc_broker_number_to_name(filter):
    broker_data = get_hk_broker_data(filter)
    if len(broker_data) > 0:
        return ImmutableDict(
            dict(zip(broker_data.broker_number, broker_data.broker_name))
        )
    else:
        return ImmutableDict()
