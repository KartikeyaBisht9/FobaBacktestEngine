from foba_backtest_engine.analysis_utils.target_function.delayed_rws import adjust_for_lunch_inplace, calculate_delayed_rws_midspot
from foba_backtest_engine.analysis_utils.feature_functions.bbov import smooth_bbov, raw_bbov
...
def get_slipped_fields(df, bookId, exclude_lunch = True, pnl_slippage_times = [5,15,30,60,120,240,300,600,900,1800,3600,7200], bbov_weights=[1,6,6,1], smooth_bbov_alpha = 0.10, bbov_interval_s=10):
    createdNanos_, received_, timestamp_ = df["createdNanos_"].values, df["received_"].values, df["timestamp_"].values
    if exclude_lunch:
    ...
    midspot, rws, pnl_slippage_times = df["midspot"].values, df["rws"].values, np.array(pnl_slippage_times)
    ...
    volumes = df[[f"bids_{x}_volume_" for x in range(max(5, bbov_length/2))]+[f"asks_{x}_volume_" for x in range(max(5, bbov_length/2))]].to_numpy()
    ...
    combined_array = np.hstack((ts, book_id_array, even_id_array, bbov.reshape(-1, 1), smooth.reshape(-1, 1), result))

    result_df = pd.DataFrame(combined_array, columns=["createdNanos_", "bookId_", "event_id_match", "raw_bbov", "smooth_bbov"]+[f'midspot_{x}' for x in pnl_slippage_times] + [f'rws_{x}' for x in pnl_slippage_times])
    result_df["bookId_"] = result_df["bookId_"].astype(int).astype("str")
    result_df["event_id_match"] = result_df["event_id_match"].astype("int")
    result_df[["rws", "rws_skew", "optiver_xgb_val", "midspot"]] = df[["rws", "rws_skew", "optiver_xgb_val", "midspot"]]
...
def annotate_slippages(
    ...
    enriched_df["rws"] = (enriched_df.bids_0_volume_*enriched_df.asks_0_price_+enriched_df.asks_0_volume_*enriched_df.bids_0_price_)/(enriched_df.bids_0_volume_+enriched_df.asks_0_volume_)
    enriched_df["rws_skew"] = (enriched_df.bids_0_volume_)/(enriched_df.bids_0_volume_+enriched_df.asks_0_volume_)-0.50
    enriched_df["optiver_xgb_val"] = optiver_xgb_valuation()
    enriched_df["midspot"] = (enriched_df.asks_0_price_+enriched_df.bids_0_price_)/2.0
    ...
    for book in enriched_df["bookId_"].unique():
        book_enriched = enriched_df[enriched_df["bookId_"]==book].reset_index(drop = True)
    ...
    slippage_columns = [c for c in future_valuation.columns if c not in ["bookId_", "event_id_match", "createdNanos_"]]
    future_valuation[slippage_columns] = future_valuation[slippage_columns].ffill().bfill()

    FeedStateSlippages = namedtuple("FeedStateSlippages", [c for c in future_valuation.columns if c != "event_id_match"])
    ...
    def items():
        for event_id, event in feed_states.items():
            try:
                ...
                raise AssertionError("enriched slippages should have same number of rows as feed states!")
